...
public class AES {
    ...
    private static byte[][] generateSubkeys(byte[] key) {
		...
		//nb = 4 Nk = key.length /4 nr = NK +6
        // es wird ein neues byte array erzeugt
        byte[][] tmp = new byte[Nb * (Nr + 1)][4];

        int i = 0;
        while (i < Nk) {
            // es werden jedes mal 4 bit des schlüssels in temp geschrieben dh. der schlüssel wird in 4.bit blöcke aufgeteilt
            tmp[i][0] = key[i * 4];
            tmp[i][1] = key[i * 4 + 1];
            tmp[i][2] = key[i * 4 + 2];
            tmp[i][3] = key[i * 4 + 3];
        ...
        while (i < Nb * (Nr + 1)) {
			// es wird ein neues 4bit langes byte erzeugt 

            byte[] temp = new byte[4];

            //es werden 4 bit ausgelesen
            for (int k = 0; k < 4; k++) {
                temp[k] = tmp[i - 1][k];
            }
